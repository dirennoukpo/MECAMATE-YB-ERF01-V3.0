...
void process_sensor_output()
{
    ...
    if (quat6_data_was_set == 1)
    {
        if (hal.report & PRINT_GRV)
        {
            ...
            values[0] = -asin(2 * (q02 - q13));                          // Roll angle
            values[1] = atan2(2 * (q03 + q12), (1 - 2 * (q22 + q33)));   // Pitch angle
            values[2] = atan2(2 * (q01 + q23), (1 - 2 * (q11 + q22)));   // Yaw angle
...
//Initialize the gyroscope-related parameters
int ICM_20948_Init(void)
{
    ...
    tx = 0x80;
    result |= inv_serial_interface_write_hook(0x06, 1, &tx); //reset
    delay_ms(100);
    // App_Delay_ms(100);
    tx = 0x00;
    result |= inv_serial_interface_write_hook(0x7f, 1, &tx);
...
// Retrieve the data from the ICM20948
...
// Return whether the ICM20948 initialization is complete
...
{
    return g_icm_data.orientation[0];         // returns radians
    // return g_icm_data.orientation[0]*RtA;  // returns degrees
}

// Get the current pitch angle
float ICM20948_Get_Pitch_Now(void)
{
    return g_icm_data.orientation[1];         // returns radians
    // return g_icm_data.orientation[1]*RtA;  // returns degrees
}

// Get the current yaw angle
float ICM20948_Get_Yaw_Now(void)
{
    return g_icm_data.orientation[2];         // returns radians
    // return g_icm_data.orientation[2]*RtA;  // returns degrees
}

// ICM20948 data reading handler
void ICM20948_Read_Data_Handle(void)
{
    if (hal.new_gyro == 1)
    {
        hal.new_gyro = 0;
        fifo_handler();    //the handler can be placed in an interrupt
...
// Send raw data to the main controller
void ICM20948_Send_Raw_Data(void)
{
	...
	data_buffer[3] = FUNC_REPORT_ICM_RAW; // function byte
...
// Send attitude angle data to the main controller, unit: radians
