...
#define	ICM20948_CS  PBout(12)   //Chip select signal
...
// Roll angle Φ (Roll),   range is (-90 to 90)
// Pitch angle θ (Pitch), range is (-180 to 180)
// Yaw angle ψ (Yaw),     range is (-180 to 180)
typedef struct
{
	float accel_float[3];
	float gyro_float[3];
	float compass_float[3];
	float orientation[3];    // Radians [Roll, Pitch, Yaw]
	float temperature;
}icm20948_data_t;
