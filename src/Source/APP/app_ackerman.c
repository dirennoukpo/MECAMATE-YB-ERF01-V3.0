...
// Ackermann angle related variables
int akm_servo_angle = 0;

// Ackermann angular speed related variables
float akm_speed_angle = 0;
float akm_angle = 0;

// Ackermann front wheel default angle; the turning angle is relative to it, increased or decreased.
...
// Control the Ackermann servo turning angle, left negative right positive. angle=±45
...
// Control the Ackermann servo turning angle, and update the left/right motor speeds according to the angle, left negative right positive. angle=±45
...
// Get the Ackermann front wheel servo steering angle
int16_t Ackerman_Get_Steer_Angle(void)
{
    return -akm_servo_angle;
}

// Get the Ackermann front wheel servo default angle
uint16_t Ackerman_Get_Default_Angle(void)
{
    return g_ackerman_default_angle;
}

// Set the Ackermann front wheel servo default angle
...
// X axis speed (forward positive, backward negative: ±1800), Y axis servo angle (left positive, right negative: ±45), Z axis speed (angular speed, left positive, right negative: ±3000)
...
// Control the motion state of the Ackermann car
...
// Return the current Ackermann front wheel servo default angle to the host.
void Ackerman_Send_Default_Angle(void)
{
	...
	data_buffer[3] = FUNC_AKM_DEF_ANGLE; // Function byte
