...
uint8_t SBUS_RX_BUF[SBUS_RECV_MAX]; //Receive buffer, at most USART_REC_LEN bytes.
...
// Parse the SBUS data and convert it into values.
...
// SBUS receive data processing handler
void SBUS_Handle(void)
{
    ...
    if (sbus_new_cmd)
    {
        ...
        if (value_x == 0 && value_y == 0 && value_z == 0)
        {
            ...
            if (stop_flag == stop_count)
            {
                // Brake timeout, switch to free stop.
