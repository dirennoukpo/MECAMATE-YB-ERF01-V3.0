...
// Switch to beep the buzzer once when a message is received. Off by default, automatically turned on after system initialization completes.
uint8_t g_can_beep_state = 0;


// Parse the data received over CAN 
// Protocol format: ID|FUNC|①|②|③|④|⑤|CHECK
...
// Get the prescaler value corresponding to the baud rate
static uint8_t CAN_Get_Prescaler(EN_CAN_BAUDRATE baudrate)
{
    uint8_t prescaler = 4; // Default is 1000Kbps
...
// Configure the CAN interrupt settings
...
void CAN_Config_Init(EN_CAN_BAUDRATE baudrate)
{
    ...
    // CAN baud rate calculation: baud=Fpclk/(BRP+1)/(SJW_+BS1_+BS2_)
...
// CAN FIFO0 receive interrupt handler
...
void CAN_Send_Data(CAN_Data_t* data)
{
    ...
    TxMsg.RTR = CAN_RTR_DATA;  // Frame type: data frame, remote frame
    TxMsg.IDE = CAN_ID_STD;    // Frame format: standard frame, extended frame
    TxMsg.DLC = 8;
    for (i = 0; i < 8; i++)
    {
...
// Switch to beep the buzzer once when a message is received
